// routes/advancedTaskExport.js
// Advanced Feature: Advanced Task Export (step 93)
...
router.post('/', authenticateJWT, async (req, res) => {
    try {
        ...
        res.json({ success: true, exportId, message: 'Advanced task export started.' });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Export failed.', error: error.message });
    }
});

// GET /api/task-export/advanced/status
